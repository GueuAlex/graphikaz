...
interface SelectedRollupViewProps {
  selectedRollup: RollupProps;
  setSelectedRollup: (selectedRollup: RollupProps | undefined) => void;
}
const SelectedRollupView: React.FC<SelectedRollupViewProps> = ({
  selectedRollup,
  setSelectedRollup,
}) => {
  const unselectedRollups = rollups.filter(
    (flyer) => flyer.type === selectedRollup.type
  );
  const paragraphs = selectedRollup.short_description.split("\n");
  ...
  const [paperType, setPaperType] = useState<MetaDataProps>(rollupSupport[0]); //);
  ...
  const totalPrice =
    (selectedRollup.base_price + printingSide.price + paperType.price) *
      quantity +
  ...
  const order: ImpressOrderProps = {
    ...
    impressable: selectedRollup.title,
    impressable_format: selectedRollup.format,
    impressable_type: selectedRollup.type,
    ordering_at: null,
    quantity: quantity,
    amount: totalPrice,
    ...
    <div className="selected-flyer-container">
        ...
        <div className="flyer-details ">
          <div className="flyer-details-left  flex-1">
            <div className="sticky-left ">
              <div className="cover-container w-full relative">
                <div
                  className="go-back"
                  onClick={() => setSelectedRollup(undefined)}
                >
                  {"<- Retour"}
                </div>
                <Image src={selectedRollup.cover} alt={selectedRollup.title} />
              </div>
              <div className="unselected-container">
                <h3>SÉLECTIONNER UN STYLE DE FLYERS</h3>
                <div className="unseleted-content w-full">
                  {unselectedRollups.map((rollup) => (
                    <div
                      className={`unselected-f ${
                        rollup.id === selectedRollup.id ? "active" : ""
                      }`}
          ...
          <div className="flyer-details-right flex-1">
            ...
            <div className="customisation">
              <h1>Customiser votre flyers {selectedRollup.title}</h1>
              ...
              <form action="" className="w-full" onSubmit={handleClick}>
                ...
                <div className="label-input mt-2">
                  ...
                  <select
                    className=" w-full select"
                    onChange={(event) =>
                      handleSelectChange(event, setPaperType, rollupSupport)
                    }
                  >
                    {rollupSupport.map((paper) => (
      ...
      <div className="selected-flyer-desc">
        <div className="desc-container">
          <h2>Concernant {selectedRollup.title}</h2>
          <div className="descripter">
            ...
            <div className="full-desc">
              {selectedRollup.full_description.map((desc) => {
          ...
          <div className="descripter">
            ...
            <div className="">
              <ul className="px-[16px]">
                {selectedRollup.caracteristics.map((caract) => (
...
export default SelectedRollupView;
