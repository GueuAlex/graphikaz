...
const Freelancers = () => {
  ...
  const [visibleItems, setVisibleItems] = useState(
    categoriesTab.slice(0, itemsPerPage)
  );
  const handleShoMore = () => {
    ...
    <Link href={`${prestator.profilLink}${prestator.fullName}`}>
      <div className="freelancer-card hover:shadow">
        <div className="favorite-heart">
          <i className="ri-heart-line text-[0.8rem]"></i>
        </div>
        ...
        <div className="competences">
          {prestator.competence.slice(0, 2).map((competence, index) => {
            return <div className="competence">{competence}</div>;
          })}{" "}
        ...
        <button type="button">
          View profil <i className="ri-arrow-right-up-line"></i>
        </button>
      </div>
    </Link>
  );
