  ...
  if (req.method === "POST") {
    ...
    if (transaction) {
      ...
      /* const { data, error } = await resend.emails.send({
        from: "GRAPHIKAZ <onboarding@resend.dev>",
       
        to: ["klagueu@gmail.com"],
        subject: "Accusé de reception",
        react: NewGraphicServOrderEmail({
          items: filteredItems,
      ...
      //consummer email body
      const emailHtml = render(
        NewGraphicServOrderEmail({
          items: filteredItems,
          packType: packType as GraphicServPackType,
          serviceTitle: libelle,
          image: covers[0],
